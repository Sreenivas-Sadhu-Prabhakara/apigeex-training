/* widgets.js — interactive learning widgets for the Apigee X training site.
 *
...
(function () {
  ...
  register("sequence", function (el, cfg) {
    ...
    var lane = 150, W = Math.max(520, actors.length * lane), rowH = 52, top = 66, H = top + steps.length * rowH + 24;
    var ax = {};
  ...
  register("curriculummap", function (el) {
    ...
    fetch("assets/curriculum.json").then(function (r) { return r.json(); }).then(function (cur) {
      ...
      var pillars = [];
      Object.keys(cur.days).forEach(function (k) {
        var p = cur.days[k].pillar; if (pillars.indexOf(p) < 0) pillars.push(p);
      });
      var filterBtns = '<button class="cmap-filter on" data-p="all">All pillars</button>' +
        pillars.map(function (p) { return '<button class="cmap-filter" data-p="' + esc(p) + '">' + esc(p) + "</button>"; }).join("");
      var weeks = cur.weeks.map(function (w) {
        var cards = w.days.map(function (d) {
          var m = cur.days[String(d)];
          var isDone = done.indexOf(d) >= 0;
          return '<a class="cmap-card pill-' + esc(m.pillar.replace(/\s+/g, "")) + (isDone ? " done" : "") +
            '" data-p="' + esc(m.pillar) + '" href="day-' + (d < 10 ? "0" + d : d) + '.html" ' +
            'title="' + esc(m.objective) + '">' +
            '<span class="cmap-day">Day ' + (d < 10 ? "0" + d : d) + '</span>' +
            '<span class="cmap-check">&#10003;</span>' +
            '<span class="cmap-title">' + esc(m.title) + "</span>" +
            '<span class="cmap-pillar">' + esc(m.pillar) + "</span></a>";
        }).join("");
        return '<div class="cmap-week"><div class="cmap-week-title">' + esc(w.title) +
          '<span class="cmap-week-theme">' + esc(w.theme) + "</span></div><div class='cmap-grid'>" + cards + "</div></div>";
      }).join("");
      el.innerHTML = '<div class="cmap-filters">' + filterBtns + "</div>" + weeks;
      el.querySelectorAll(".cmap-filter").forEach(function (b) {
        b.addEventListener("click", function () {
          el.querySelectorAll(".cmap-filter").forEach(function (x) { x.classList.remove("on"); });
          b.classList.add("on");
          var p = b.getAttribute("data-p");
          el.querySelectorAll(".cmap-card").forEach(function (c) {
            c.classList.toggle("dim", p !== "all" && c.getAttribute("data-p") !== p);
          });
        });
      });
    }).catch(function () {
      el.innerHTML = '<div class="cmap-loading">Curriculum map loads when served over HTTP (it is live on the site).</div>';
    });
  });
